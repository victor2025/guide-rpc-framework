899999999999# guide-rpc-framework
...
- [x] 使用 `CompletableFuture` 包装接受客户端返回结果（之前的实现是通过 `AttributeMap` 绑定到 Channel 上实现的） 详见：[使用 CompletableFuture 优化接受服务提供端返回结果](使用CompletableFuture优化接受服务提供端返回结果.md)
